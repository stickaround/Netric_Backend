...
for ($i = 0; $i < $num; $i++) {
    ...
    $doc1 = new Elastica_Document(
        $i,
        ['val_i' => $i, 'val_txt' => $text, 'val_tsort' => $text, 'val_t' => gmdate("Ymd\\TG:i:s", time())]
    );
...
for ($i = 0; $i < $num_queries; $i++) {
    ...
    $queryObject->setRawQuery(['constant_score' => [ 'filter' => ["term" => ["val_text" => $term_text]] ]]);
    ...
    $queryObject->setFrom(0);
    $queryObject->setLimit(10);
    $queryObject->setRawQuery(['constant_score' => [ 'filter' => ["term" => ["val_text" => substr($term_text, 0, strlen($term_text) - 3)."*"]] ]]);
    //$type->search($queryObject);
    $tEnd = microtime(true);
    ...
    $queryObject->setFrom(0);
    $queryObject->setLimit(10);
    $queryObject->setRawQuery(['constant_score' => [ 'filter' => ["term" => ["val_number" => $term_number]] ]]);
    $type->search($queryObject);
    $tEnd = microtime(true);
    ...
    $queryObject->setFrom(0);
    $queryObject->setLimit(10);
    $queryObject->setRawQuery(['constant_score' => [ 'filter' => ["range" => ["val_number" => ["from" => ($term_number - 100), "to" => ($term_number + 100)]]] ]]);
    $type->search($queryObject);
    $tEnd = microtime(true);
