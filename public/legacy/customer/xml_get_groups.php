...
require_once(__DIR__ . "../../../src/AntLegacy/AntConfig.php");
	...
	{
		$result = $dbh->Query("select id, parent_id, name, color from customer_labels where parent_id='$gid' order by name DESC");
	...
	echo "<groups>";
	$result = $dbh->Query("select id, parent_id, name, color from customer_labels where parent_id is null order by name DESC");
	$num = $dbh->GetNumberRows($result);
	for ($i = 0; $i < $num; $i++)
	{
		$row = $dbh->GetNextRow($result, $i);
