<?php
	...
	*/
	$result = $dbh_acc->Query("select id, recur_id from calendar_events where recur_id is not null order by id;");
	...
	for ($i = 0; $i < $num; $i++)
	{
		...
		$obj = new CAntObject($dbh_acc, "calendar_event", $eid);
		...
		if (!$obj->isRecurring())
		{
			...
			$res2 = $dbh_acc->Query("select *, week_days[1] as day1,
									  ...
									  from calendar_events_recurring where id='$rid'");			
