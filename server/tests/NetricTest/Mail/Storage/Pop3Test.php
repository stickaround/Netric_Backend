...
class Pop3Test extends \PHPUnit_Framework_TestCase
{
    ...
    {
        if (!getenv('TESTS_NETRIC_MAIL_POP3_ENABLED')) {
            $this->markTestSkipped('Netric_Mail POP3 tests are not enabled');
        }

        $this->_params = ['host'     => getenv('TESTS_NETRIC_MAIL_POP3_HOST'),
    ...
    {
        if (!getenv('TESTS_NETRIC_MAIL_POP3_SSL')) {
            return;
        }
    ...
    {
        if (!getenv('TESTS_NETRIC_MAIL_POP3_TLS')) {
            return;
        }
    ...
    {
        $this->_params['port'] = getenv('TESTS_NETRIC_MAIL_POP3_INVALID_PORT');
    ...
    {
        $this->_params['port'] = getenv('TESTS_NETRIC_MAIL_POP3_WRONG_PORT');
...
/*
    ...
    {
        $mail = new Netric_Mail_Storage_Pop3($this->_params);
