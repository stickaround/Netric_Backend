...
class MaildirOldMessage extends Storage\Maildir
{
     ...
     * @var string
     */
    protected $_messageClass = 'Netric\Mail\Storage\Message';
}
...
class MaildirMessageOldTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function setUp()
    {
        ...
        if (!getenv('TESTS_NETRIC_MAIL_MAILDIR_ENABLED')) {
            $this->markTestSkipped('You have to unpack maildir.tar in Netric/Mail/_files/test.maildir/ '
                                 . 'directory before enabling the maildir tests');
            return;
        }

        if ($this->_tmpdir == null) {
            if (getenv('TESTS_NETRIC_MAIL_TEMPDIR') != null) {
                $this->_tmpdir = getenv('TESTS_NETRIC_MAIL_TEMPDIR');
            } else {
