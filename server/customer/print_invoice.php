<?php
	...
	{
		$result = $dbh->Query("select company_logo, company_name, company_slogan, notes_line1, notes_line2, footer_line1 
								from customer_invoice_templates where id='".$_POST['template_id']."'");
		if ($dbh->GetNumberRows($result))
		{
			$row = $dbh->GetNextRow($result, 0);

			$g_company_logo = $row['company_logo'];
			$g_company = $row['company_name'];
			$g_slogan = $row['company_slogan'];
			$g_notes_line1 = $row['notes_line1'];
			$g_notes_line2 = $row['notes_line2'];
			$g_footer_line1 = $row['footer_line1'];
		}
	}
	else
	{
	...
	/*
	if ($_POST['send_to'] && $_POST['send_to_cbill']!='t')
	{
	...
	if ($_POST['customer_id'])
	{
		...
		if ($obj->getValue("billing_street"))
		{
			$send_to .= $obj->getValue("billing_street")."\n";
			$send_to .= $obj->getValue("billing_city");
			$send_to .= $obj->getValue("billing_state")." ";
			$send_to .= $obj->getValue("billing_zip");
		}
	...
	/*
	$pdf->addText(70, 640, 11, "Sky Stebnicki");
