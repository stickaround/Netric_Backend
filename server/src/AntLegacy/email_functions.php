...
function EmailReplaceInlineImages(&$dbh, $MID, &$tmp_body)
{
	...
	for ($i = 0; $i < $num; $i++)
	{
		...
		$tmp_body = str_replace("src=\"cid:".str_replace("<", '', str_replace(">", '', $row['content_id']))."\"", 
						"src=\"/public/legacy/email/attachment.awp?attid=".$row['id']."\"", $tmp_body);
		$tmp_body = str_replace("src='cid:".str_replace("<", '', str_replace(">", '', $row['content_id']))."'", 
						"src=\"/public/legacy/email/attachment.awp?attid=".$row['id']."\"", $tmp_body);
	}
	$dbh->FreeResults($result);
	...
	for ($i = 0; $i < $num; $i++)
	{
		...
		case 'text/plain':
			if ($row['disposition'] == 'attachment')
			{
				$att_buf.="<br><table border='0'><tr><td>
							<a href='/public/legacy/email/attachment.awp?attid=".$row['id']."&disposition=attachment' target='_blank'>
							<img border='0' src='/images/icons/email_attachments/generic.gif'></a>
							</td><td valign='top'>
							Filename: <a href='/public/legacy/email/attachment.awp?attid=".$row['id']."&disposition=attachment' 
		...
		case 'application/ms-tnef':
			...
			for ($m = 0; $m < count($tnef_files); $m++)
			{
				...
				{
					$lnk = "/public/legacy/email/attachment.awp?attid=".$row['id']."&tnefatt=$m&disposition=attachment";
					$att_buf.="<br><table border='0'><tr><td>
		...
		case 'image/pjpeg':
			if ($row['disposition'] == 'attachment')
			{
				...
				{
					//$fid = $row['file_id'];
					$lnk = "/public/legacy/email/attachment.awp?attid=".$row['id'];
					$lnk_thumb = "/public/legacy/email/attachment.awp?attid=".$row['id'];
				}
				...
				$m_body .= "<br><div><img border='0' src='/public/legacy/email/attachment.awp?attid=".$row['id']."'></div>";
...
function EmailGetBodyAntfiles(&$dbh, &$body)
{
	...
	$arr_check = array("src=\"/public/legacy/email/attachment.awp?attid="=>'"',
					   "src='/public/legacy/email/attachment.awp?attid="=>"'");
	foreach ($arr_check as $chec_beg=>$check_end)
	{
		$cur_pos = 0;
		while (1) 
...
function EmailEmbedBodyAntfiles(&$dbh, &$body, &$antfiles)
{
	...
	foreach ($antfiles as $emfile)
	{
		...
		case "attachment":
			$cont_id = $emfile[4];
			$body = str_replace("src=\"/public/legacy/email/attachment.awp?attid=".$emfile[1]."\"", "src=\"$cont_id\"", $body);
			$body = str_replace("src='/public/legacy/email/attachment.awp?attid=".$emfile[1]."'", "src='$cont_id'", $body);
			break;
		}
	}
