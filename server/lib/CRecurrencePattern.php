<?php
	...
	*/
	...
	class CRecurrencePattern
	{
		// Recurrance variables
		var $type;
		var $interval; 			// Inverval every (num) days/weeks/months/years - defaults to 1
		var $dateStart; 		// Recur starts
		var $dateEnd; 			// Recur ends. If null then forever
		var $timeStart; 		// Time starts with time zone name
		var $timeEnd; 			// Time starts with time zone name
		var $fAllDay; 			// bool = all day event or not
		var $dayOfMonth;
		var $monthOfYear;
		var $dayOfWeekMask;		// S,M,T,W,T,F,S
		var $duration; 			// In minutes
		var $instance; 			// The 1st-5th(last) RECUR_MONTHNTH and RECUR_YEARNTH
		...
		**************************************************************************/
		function save()
		{
			...
			$toupdate['type'] = $dbh->EscapeNumber($this->type);
		...
		***************************************************************************/
		function isChanged()
		{
			...
			if ($this->arrChangeLog['interval'] != $this->interval)
				return true;
			else if ($this->arrChangeLog['type'] != $this->type)
				return true;
