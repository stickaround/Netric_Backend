<?php
	...
	*	ant.php
	*
	*	Purpose: This document contains global functions and definitions & should be included
	...
	**************************************************************************************/
	...
	// Default color values (replicated in lib/js/Ant.js)
	// -------------------------------------------------------
	$G_GROUP_COLORS = array(
		// Softer
		...
		// Harsher/Bolder
		array("Blue", "0000FF"), array("Red", "FF0000"), array("Green", "008000"), 
		array("Purple", "800080"), array("Yellow", "FFFF00"),
		array("Orange", "FFA500"), array("Violet", "EE82EE"), array("Silver", "C0C0C0"), 
		array("Gold", "FFD700"), array("Gray", "808080"),
		array("Pink", "FFC0CB"), array("Fuscia", "FF00FF"), array("Light Blue", "ADD8E6"),
		array("Sky Blue", "87CEEB"), array("Aqua", "00FFFF"), array("Khaki", "F0E68C")
	);

	$G_THEMES = array(
		array("name", "Title", "css")
	);
	...
	/*************************************************************************************
	*	Description:	antGetRandColor
	*
	*	Purpose:		Get a random color to use from G_GROUP_COLORS
	*
	*	Arguments:		1. $prevUsed - array: colors previously used
	**************************************************************************************/
	function antGetColorRand($prevUsed=1)
	{
		if ($prevUsed <= count($G_GROUP_COLORS))
		{
			$prevUsed++;
			return $G_GROUP_COLORS[$prevUsed-2][1];
		}
		else
		{
			$prevUsed = 1;
			return $G_GROUP_COLORS[$prevUsed-1][1];
		}
	}

	/*************************************************************************************
	*	Description:	antGetColors
	*
	*	Purpose:		Get list of color themes to work with
	*
	*	Arguments:		1. $prevUsed - array: colors previously used
	**************************************************************************************/
	function antGetColors($theme=null)
	{
		$ret = array();
		global $G_GROUP_COLORS;

		foreach ($G_GROUP_COLORS as $clr)
			$ret[] = $clr;
...
/*
* Corrects the date into server's timezone
*/
function correctTimezone($date)
{
    $daylight = 60*60*24*date("I"); // Set the daylight saving time
    $diffSec = date("Z") + (60*60*2); // Set the GMT difference
    $ts = strtotime($date); // Create the timestamp
    return date("m/d/Y h:i A", $ts + $daylight + $diffSec);
}
?>
