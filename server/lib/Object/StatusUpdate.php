 ...
 * functions for posting status updates on objects
 *
 * Example
 * <code>
 * </code>
...
/**
 * Object extensions for managing status updates which are essentially the same thing as comments
 */
class CAntObject_StatusUpdate extends CAntObject
{
	/**
	...
	{
		parent::__construct($dbh, "status_update", $cid, $user);
	}
	...
	public function sendNotifications($notify)
	{
		...
		foreach ($notify as $sendto)
		{
			...
			if ($this->getValue('obj_reference'))
				$referenced_obj = CAntObject::decodeObjRef($this->getValue('obj_reference'));
				...
				if ($obj_parts)
				{
					switch ($obj_parts['obj_type'])
					{
						case 'user':
							...
							$link = $this->getAccBaseUrl()."/obj/status_update/".$this->id;
							break;
			...
			if ($eml && !in_array($eml, $sentTo))
			{
				...
				*/
				...
				$headers['To'] = $eml;
				$headers['Subject'] = "Added Status Update";
				$body = "By: $from\r\n-----------------------------------------------------\r\n\r\n";
