...
abstract class AbstractAction implements ErrorAwareInterface
{
     ...
     *
     * For example:
     *  'test' will load Netric\WorkFlowLegacy\Action\TestAction.php
     *  'email' will load Netric\WorkFlowLegacy\Action\EmailAction.php
     *
     * @var string
     */
    private $type = null;
     ...
     */
    abstract public function execute(WorkFlowLegacyInstance $workFlowInstance);
    ...
    public function addAction(ActionInterface $actionToAdd)
    {
        ...
        for ($i = 0; $i < count($this->removedChildActions); $i++) {
            if ($actionToAdd === $this->removedChildActions[$i] || ($actionToAdd->getWorkFlowLegacyActionId() != null
                && $actionToAdd->getWorkFlowLegacyActionId() === $this->removedChildActions[$i]->getWorkFlowLegacyActionId())) {
    ...
    {
        if ($action === $this || ($action->getWorkFlowLegacyActionId() && $action->getWorkFlowLegacyActionId() === $this->getWorkFlowLegacyActionId())) {
            return true;
        }
