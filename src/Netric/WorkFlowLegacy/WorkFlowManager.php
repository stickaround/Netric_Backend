...
use Netric\WorkFlowLegacy\DataMapper\DataMapperInterface;
use Netric\Log\Log;
 ...
 * Workflow service
 */
class WorkFlowLegacyManager extends AbstractHasErrors
{
    /**
    ...
    /**
     * Start WorkFlowLegacys for an entity based on an action
     *
     * @param EntityInterface $entity The entity being acted on
     * @param string $event One of WorkFlowLegacy::EVENT_
     */
    public function startWorkFlowLegacys(EntityInterface $entity, $event)
    {
        $objType = $entity->getDefinition()->getObjType();

        // Get active WorkFlowLegacys for this entity
        $workFlows = $this->workFlowDataMapper->getWorkFlowLegacys($objType, true, $event);
        foreach ($workFlows as $workFlow) {
            // Check if conditions match
            if ($this->workFlowConditionsMatch($workFlow, $entity)) {
                $this->startWorkFlowLegacyInstance($workFlow, $entity);
            }
        }
    }

    /**
     * Run and execute WorkFlowLegacys that run periodically (daily)
     */
    public function runPeriodicWorkFlowLegacys()
    {
        // Get all daily workFlows
        $workFlows = $this->workFlowDataMapper->getWorkFlowLegacys(null, true, "daily");
        foreach ($workFlows as $workFlow) {
            ...
            $this->log->info("Ran periodic WorkFlowLegacy " . $workFlow->getWorkFlowLegacyId() . " on $num entities");
        }
    }

    /**
     * Force starting a WorkFlowLegacy against an entity
     *
     * @param EntityInterface $entity Entity to execute WorkFlowLegacy against
     * @param int $wid The unique id of the WorkFlowLegacy to start
    ...
    public function startWorkflowById(EntityInterface $entity, $wid)
    {
        ...
        if (!$workFlow) {
            throw new \RuntimeException("WorkFlowLegacy $wid does not exist");
        }

        if ($workFlow->getObjType() != $entity->getDefinition()->getObjType()) {
            throw new \RuntimeException(
                "WorkFlowLegacy id $wid only runs against objType '" . $workFlow->getObjType() . "'" .
    ...
    /**
     * Get a WorkFlowLegacy by id
     *
     * @param id $id
     * @return WorkFlowLegacy
     */
    public function getWorkFlowLegacyById($id)
    {
     ...
     * @return WorkFlowLegacy[]
     */
    public function getWorkFlowLegacys($objType = null)
    {
        return $this->workFlowDataMapper->getWorkFlowLegacys($objType);
    }

    /**
     * Save workflow
     *
     * @param WorkFlowLegacy $workFlow
     * @return bool true on success, false on failure
     */
    public function saveWorkFlowLegacy(WorkFlowLegacy $workFlow)
    {
     ...
     *
     * @param WorkFlowLegacy $workFlow The WorkFlowLegacy we would like to know about
     * @param EntityInterface $entity The entity we are checking against the WorkFlowLegacy conditions
     * @return bool true if the entity matches, or false if it does not
     */
    private function workFlowConditionsMatch(WorkFlowLegacy $workFlow, EntityInterface $entity)
    {
     ...
     *
     * @param WorkFlowLegacy $workFlow The WorkFlowLegacy we would like to run
     * @param EntityInterface $entity The entity we are running on
     */
    private function startWorkFlowLegacyInstance(WorkFlowLegacy $workFlow, EntityInterface $entity)
    {
        $workFlowInstance = new WorkFlowLegacyInstance($workFlow->getWorkFlowLegacyId(), $entity);
        $this->workFlowDataMapper->saveWorkFlowLegacyInstance($workFlowInstance);
     ...
     * @param WorkFlowLegacyInstance $workFlowInstance The instance of the workflow being executed
     * @param bool $purgeScheduled If true clear the scheduled task on successfully executed
     */
    private function executeAction(ActionInterface $action, WorkFlowLegacyInstance $workFlowInstance, $purgeScheduled = false)
    {
        if ($action->execute($workFlowInstance)) {
            // Log what just happened for troubleshooting
            $this->log->info("Executed action " . $action->getWorkFlowLegacyActionId() . " against instance " . $workFlowInstance->getWorkFlowLegacyInstanceId());

            // Delete any scheduled tasks if set
            if ($purgeScheduled) {
                $this->workFlowDataMapper->deleteScheduledAction($workFlowInstance->getWorkFlowLegacyInstanceId(), $action->getWorkFlowLegacyActionId());
            }
            ...
            // Log the error
            $this->log->error("Failed to execute " . $action->getWorkFlowLegacyActionId() . ": " . $action->getLastError()->getMessage());
        }
    }
     ...
     *
     * @param WorkFlowLegacy $workFlow The WorkFlowLegacy with conditions we use for finding entities
     * @return Results EntityQuery results from the query if found
     */
    private function getEntitiesThatMatchConditions(WorkFlowLegacy $workFlow)
    {
