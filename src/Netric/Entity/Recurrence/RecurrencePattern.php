...
class RecurrencePattern implements ErrorAwareInterface
{
    ...
    public function isChanged()
    {
        ...
        if ($this->arrChangeLog['interval'] != $this->interval) {
            return true;
        } elseif ($this->arrChangeLog['type'] != $this->recurType) {
            return true;
        } elseif ($this->arrChangeLog['date_start'] != $this->dateStart) {
            return true;
        } elseif ($this->arrChangeLog['date_end'] != $this->dateEnd) {
            return true;
        } elseif ($this->arrChangeLog['dayofmonth'] != $this->dayOfMonth) {
            return true;
        } elseif ($this->arrChangeLog['monthofyear'] != $this->monthOfYear) {
            return true;
        } elseif ($this->arrChangeLog['instance'] != $this->instance) {
            return true;
        } elseif ($this->arrChangeLog['dayofweekmask'] != $this->dayOfWeekMask) {
            return true;
        }
