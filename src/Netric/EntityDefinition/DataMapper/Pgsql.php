...
class Pgsql extends DataMapperAbstract implements EntityDefinitionDataMapperInterface
{
    /**
     * The database host
     *
    ...
    private function createObjectTable($objType, $typeId)
    {
        ...
        if (!$dbh->tableExists($tables[0])) {
            ...
            $dbh->query("CREATE UNIQUE INDEX IF NOT EXISTS " . $tables[0] . "_id_idx");
        }
        ...
        foreach ($tables as $tbl) {
            ...
            if (!$dbh->indexExists($tbl . "_tse_idx")) {
                $dbh->query("CREATE INDEX " . $tbl . "_ts_entered_idx
							  ON $tbl (ts_entered);");
            }

            if (!$dbh->indexExists($tbl . "_tsu_idx")) {
