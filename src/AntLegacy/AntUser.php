...
require_once("src/AntLegacy/Stats.php");
require_once("src/AntLegacy/CAntObject.php");
require_once("src/AntLegacy/AntMail/Account.php");
...
class AntUser
{
	...
	public function __construct($dbh, $id=null, $ant=null)
	{
        ...
        if(isset($_GET['auth']))
		{
			...
			if ($this->id)
				$this->name = $this->UserGetName($dbh, $this->id);
		}
		else
		{
        ...
        if ($this->id!=null)
		{
			...
			$this->themeName = $this->UserGetTheme($this->dbh, $this->id, 'name');
			$this->themeCss = $this->UserGetTheme($this->dbh, $this->id, 'css');
	...
	function getEmailUserName()
	{
		...
		if (!$this->emailUserName)
			$this->emailUserName = $this->EmailGetUserName($dbh, $this->id);
	...
	public function getEmail($dbh = null, $userid = null)
	{
		...
		if (isset($this) && get_class($this) == __CLASS__)
		{
			if (!$this->email)
			{
				if ($this->getValue("email"))
					...
					$this->email = $this->UserGetEmail($dbh, $userid);
			}
		...
		{
			return $this->UserGetEmail($dbh, $userid);
		}
	}
	...
	public function getSetting($key)
	{
		...
		if (is_numeric($this->id) && $key)
		{
			...
			if ($cval === false)
			{
				$query = "select value from settings where user_id='".$this->id."' and name='$key'";
				$result = $dbh->Query($query);
				if ($dbh->GetNumberRows($result))
				{
	...
	public function setSetting($key, $value)
	{
		...
		if (is_numeric($this->id) && $key)
		{
			...
			$result = $dbh->Query("select id from settings where name='$key' and user_id='".$this->id."' and value='$value'");
			if ($dbh->GetNumberRows($result))
			{
				$dbh->FreeResults($result);
				return false;
			...
			$result = $dbh->Query("select id from settings where name='$key' and user_id='".$this->id."'");
			if ($dbh->GetNumberRows($result))
			{
				$row = $dbh->GetNextRow($result, 0);
				$dbh->FreeResults($result);
				$dbh->Query("update settings set value='".$dbh->Escape($value)."' where id='".$row['id']."'");
			}
			else
			{
				$dbh->Query("insert into settings(user_id, value, name) values('".$this->id."', '".$dbh->Escape($value)."', '$key')");
			}
    ...
    */
    ...
    public function UserPrefArrDelete($dbh, $user_id, $prefname, $item)
    {
        $vals = UserGetPref($dbh, $user_id, $prefname);
        if ($vals)
        {
    ...
    // Reset all values
    public function UserPrefArrSet($dbh, $user_id, $prefname, $arr)
    {
    ...
    public function UserPrefArrDefault($dbh, $user_id, $prefname, $arr)
    {
        if (is_numeric($user_id) && $prefname && is_array($arr))
        {
            $vals = UserGetPref($dbh, $user_id, $prefname);
    ...
    public function UserGetEmail($dbh, $user_id, $system=false)
    {
        if (is_numeric($user_id))
        {
    ...
    public function UserGetAccount($dbh, $user_id)
    {
        $retval = "";
    ...
    public function UserGetEmployeeInfo($dbh, $user_id, $toget)
    {
        $sVal = null;
    ...
    public function UserGetTheme($dbh, $userid, $get='css')
    {
    ...
    public function UserGetQuota($dbh, $userid)
    {
        $quota_size = "0";
    ...
    public function UserGetQuotaFree($dbh, $userid)
    {
    ...
    public function UserGetUserIdFromAddress($dbh, $address, $src="address")
    {
        $ret = null;
    ...
    public function UserLogAction($dbh, $USERID)
    {
        global $_SERVER;
    ...
    public function UserTeamsGetChildrenArray($dbh, $tid)
    {
    ...
    public function UserGetTeam($dbh, $USERID)
    {
        $ret = null;
    ...
    public function UserGetTeamName($dbh, $TID)
    {
        $ret = null;
    ...
    public function UserAccGetTeam($dbh, $ACCOUNT)
    {
        $ret = null;
    ...
    public function UserGetTeamOptionStr($dbh, $user, $selected_tid, $parent=null, $path='')
    {
        global $TEAM_ACLS;
    ...
    public function UserPublicAuth($dbh, $auth)
    {
        global $_SERVER;
