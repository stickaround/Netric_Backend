...
class CAntFsFile
{
    ...
    function __construct($dbh, $id, $name = "", $protocol = 'local')
    {
        ...
        if ($dbh->GetNumberRows($result)) {
            ...
            switch ($this->type) {
                ...
                case 'png':
                case 'gif':
                    $this->url_stream = "http://".$settings_localhost."/legacy/userfiles/file_download.awp?view=1&fid=".$this->id;
                    $this->url_download = "http://".$settings_localhost."/legacy/userfiles/file_download.awp?fid=".$this->id;
                    break;
                default:
                    $this->url_stream = "http://".$settings_localhost."/legacy/userfiles/file_download.awp?fid=".$this->id;
                    $this->url_download = "http://".$settings_localhost."/legacy/userfiles/file_download.awp?fid=".$this->id;
                    break;
            }
        }
    ...
    function read($numbytes = null, $offset = 0)
    {
        ...
        */
        ...
        } elseif ($this->remoteFile) { // Old ANS code
    ...
    private function streamAns($fileHandle = null)
    {
        ...
        $ansClient = new AnsClient("anstest.aereus.com"); // TODO: remove manual server
    ...
    private function streamOldAns($fileHandle = null)
    {
        ...
        elseif ($ans->fileVerify($this->remoteFile, $accountNumber."/".$this->id, "/userfiles")) {
            $remote_file_page = $ans->getFileUrl($this->remoteFile, $accountNumber."/".$this->id, "/userfiles", 1);
        } elseif ($ans->fileVerify($this->remoteFile, "/".$this->id, "/userfiles")) {
