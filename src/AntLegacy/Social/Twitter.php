 ...
 */
require_once("src/AntLegacy/Social/codebird/codebird.php");

/**
 * Social base class
 */
class AntSocial_Twitter extends AntSocial
{
	/**
	 * Codebird API instance
	 *
	...
	{
		\Codebird\Codebird::setConsumerKey(AntConfig::getInstance()->social['twitter_key'], AntConfig::getInstance()->social['twitter_secret']);

		$this->codebird = \Codebird\Codebird::getInstance();

		// If access token already exists then set it
		$this->accessToken = AntConfig::getInstance()->social['twitter_access_token'];
		\Codebird\Codebird::setBearerToken($this->accessToken);
		//$this->twitterUserId = $this->user->getSetting("accounts/facebook/id");
	 ...
	 *
	 * This function does not require API login
	 *
	 * Example
	 * <code>
	 * 	$twitter->getTagTweets('#cats');
	 * </code>
	 *
	 * @param string $tag The hash tag to search for
	 * @return array(array('author', 'tweet', 'message'))
	 */
	public function getTagTweets($hash_tag) 
	{
		$tweets = array();
		...
		foreach ($twelement->entry as $entry) {
			...
			$tweets[] = array(
				"author" => $author,
				"time_posted" => date('n/j/y g:i a',$time),
				"message" => $text,
			);
		}
