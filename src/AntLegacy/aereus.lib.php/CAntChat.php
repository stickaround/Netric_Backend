...
class CAntChat
{
	...
	function getQueueEid($name, $notes)
	{
		...
		$url = "http://".$this->m_server . "/legacy/chat/wapi.php?function=queue_entry_create&qid=".$this->m_queue."&name=$name&notes=".rawurlencode($notes);
	...
	function checkQueueEidStatus($eid)
	{
		...
		$url = "http://".$this->m_server . "/legacy/chat/wapi.php?function=queue_entry_get_status&queue_eid=".$eid."";
	...
	function queueIsActive()
	{
		...
		$url = "http://".$this->m_server . "/legacy/chat/wapi.php?function=queue_is_active&qid=".$this->m_queue;
