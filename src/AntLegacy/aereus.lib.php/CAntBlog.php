...
class CAntBLog
{
	...
	function CAntBLog($server, $username, $password, $fid, $aid=null) 
	{
		...
		$this->m_urlGet = "http://".$server."/legacy/objects/xml_get_object.php?auth=".base64_encode($username).":".md5($password)."&obj_type=$obj_type";
		$this->aid = $aid;
		$this->fid = $fid;
	...
	**************************************************************************************/
	function getPosts($category=null)
	{
		...
		$this->m_arrPosts = array();
		$feedReader = new CFeedReader("http://".$this->m_server."/legacy/feeds/?fid=".$this->fid, "time_entered DESC", $cond); //, null, $ALIB_WF_PUSHED
...
class CAntBlogPost
{
	...
	function CAntBlogPost($server, $username, $password, $fid, $pid)
	{
		...
		$feedReader = new CFeedReader("http://$server/legacy/feeds/?fid=$fid", "time_entered DESC", "id=".$pid); //, null, $ALIB_WF_PUSHED
