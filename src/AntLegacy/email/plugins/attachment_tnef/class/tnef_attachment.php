 ...
 * @version $Id: tnef_attachment.php,v 1.1 2009/08/06 15:48:34 administrator Exp $
 */
...
class TnefAttachment {
    ...
    function decodeTnef(&$buffer) {
        ...
        if ($tnef_signature == TNEF_SIGNATURE) {
            ...
            if ($this->tnef_debug) {
                print(sprintf("Signature: 0x%08x\nKey: 0x%04x\n", $tnef_signature, $tnef_key));
            }
            ...
            while (strlen($buffer) > 0) {
                ...
                default:
                    if ($this->tnef_debug) {
                        $len = strlen($buffer);
                        if ($len > 0) {
                            print("Invalid file format! Unknown Level $lvl_type. Rest=$len");
                        }
                    }
                    break;
                }
            ...
            if ($this->tnef_debug) {
                print("Invalid file format! Wrong signature.");
            }
        }
    }
    ...
    function tnef_decode_attribute(&$buffer) {
        ...
        case TNEF_ARENDDATA:                   // marks start of new attachment
            if ($this->tnef_debug) {
                print("Creating new File for Attachment");
            }
        ...
        case TNEF_AMCLASS:
            ...
            if ($value == 'IPM.Contact') {
                if ($this->tnef_debug) {
                    print("Creating vCard Attachment");
                }
    ...
    function extract_mapi_attrs(&$buffer) {
        ...
        while ((strlen($buffer) > 0) && ($props < $number) && (!$ended)) {
            ...
            if (($attr_type & TNEF_MAPI_MV_FLAG) != 0) {
                if ($this->tnef_debug) {
                    print("Multivalue Attribute found.");
                }
                $have_multivalue = 1;
            ...
            if (($attr_name >= 0x8000) && ($attr_name < 0xFFFE)) {      // Named Attribute
                ...
                case TNEF_MAPI_NAMED_TYPE_ID:
                    ...
                    $attr_name = $named_id;
                    if ($this->tnef_debug) {
                        print(sprintf("Named Id='0x%04x'", $named_id));
                    }
                    break;
                ...
                case TNEF_MAPI_NAMED_TYPE_STRING:
                    ...
                    if ($this->tnef_debug) {
                        print("Named Id='$named_id'");
                    }
                    break;

                default:
                    if ($this->tnef_debug) {
                        print(sprintf("Unknown Named Type 0x%04x found", $named_type));
                    }
                    break;
                }
            }
            ...
            if ($have_multivalue) {
                ...
                if ($this->tnef_debug) {
                    print("Number of multivalues=$num_multivalues");
                }
            }
$value = '';
            switch($attr_type) {
            ...
            case TNEF_MAPI_OBJECT:
                ...
                if ($num_vals > 20) {               // A Sanity check.
                    $ended = 1;
                    if ($this->tnef_debug) {
                        print("Number of entries in String Attributes=$num_vals. Aborting Mapi parsing.");
                    }
                } else {
            ...
            default:
                if ($this->tnef_debug) {
                    print("Unknown mapi attribute! $attr_type");
                }
                break;
            }
            ...
            case TNEF_MAPI_ATTACH_DATA:
                if ($this->tnef_debug) {
                    print("MAPI Found nested attachment. Processing new one.");
                }
                ...
                if ($this->tnef_debug) {
                    print("MAPI Finished nested attachment. Continuing old one.");
                }
                break;

            case TNEF_MAPI_RTF_COMPRESSED:
                if ($this->tnef_debug) {
                    print("MAPI Found Compressed RTF Attachment.");
                }
        ...
        if (($this->tnef_debug) && ($ended)) {
            ...
            for ($cnt = 0; $cnt < $len; $cnt++) {
                ...
                print(sprintf("Char Nr. %6d = 0x%02x = '%s'", $cnt, $ord, $char));
            }
        }
    }
}
