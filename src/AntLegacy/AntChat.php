...
class AntChat
{
    ...
    public function addFriend($args)
    {
        ...
        if (strpos($friendName, "@") !== false) {
            ...
            $query = "select id as user_id, team_id from objects_user where name='$friendName'";
    ...
    function getFriendList($args)
    {
        ...
        if ($teamId > 0) {
            ...
            $sql .= "select id, name as friend_name, 'localhost' as friend_server, 
					 ...
					 checkin_timestamp,
					 NULL as status 
					 from objects_user where team_id='$teamId' and id != '$userId'
        ...
        for ($i = 0; $i < $num; $i++) {
            ...
            // Sky Stebnicki: Commented this out because it was pegging the db with 220k queries in a 24
    ...
    */
    function getUserDetails($userName = null)
    {
        ...
        $query = "select users.id as user_id, users.team_id, image_id, active_timestamp, user_teams.name as team_name from objects_user left outer join user_teams on users.team_id = user_teams.id $whereClause";
        $result = $dbh->Query($query);
        $num = $dbh->GetNumberRows($result);
    ...
    */
    ...
    public function countFriendOnline($args)
    {
        ...
        if ($teamId>0) {
            $sql .= " union ";
            $sql .= "select users.active_timestamp from objects_user left outer join chat_server_session on users.id = chat_server_session.user_id 
