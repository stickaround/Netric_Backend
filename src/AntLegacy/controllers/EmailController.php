		...
		if (count($attachments))
		{
			foreach ($attachments as $att)
			{
					...
					for ($m = 0; $m < count($tnef_files); $m++)
					{
						...
						if ($file->getType() != "application/rtf")
						{
							$attData = array(
								...
								'link_download' => "/legacy/email/attachment.awp?attid=".$att->getValue('id')."&tnefatt=$m&disposition=attachment",
								'link_view' => "",
				...
				case 'image/pjpeg':
					...
					{
						$lnk = "/legacy/email/attachment.awp?attid=".$att->getValue('id');
					...
					else
					{
						...
						$lnk_thumb = "/legacy/userfiles/getthumb_by_id.awp?fid=".$att->getValue('file_id')."&iw=100";
					}
