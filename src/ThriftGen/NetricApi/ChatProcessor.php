...
class ChatProcessor
{
    ...
    protected function process_notifyAbsentOfNewMessage($seqid, $input, $output)
    {
        ...
        if ($bin_accel) {
            $args = thrift_protocol_read_binary_after_message_begin(
                $input,
                '\NetricApi\Chat_notifyAbsentOfNewMessage_args',
                $input->isStrictRead()
            );
        } else {
        ...
        $this->handler_->notifyAbsentOfNewMessage($args->messageId, $args->accountId);
        ...
        if ($bin_accel) {
            ...
            $output->writeMessageBegin('notifyAbsentOfNewMessage', TMessageType::REPLY, $seqid);
            $result->write($output);
            $output->writeMessageEnd();
            $output->getTransport()->flush();
        }
