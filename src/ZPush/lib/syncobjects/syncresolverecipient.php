...
class SyncResolveRecipient extends SyncObject
{
    ...
    public function __construct()
    {
        $mapping = [
            ...
            SYNC_RESOLVERECIPIENTS_CERTIFICATES             => [  self::STREAMER_VAR      => "certificates",
                                                                        self::STREAMER_TYPE     => "SyncResolveRecipientsCertificates"]
        ];

        if (Request::GetProtocolVersion() >= 14.0) {
            $mapping[SYNC_RESOLVERECIPIENTS_AVAILABILITY]   = [  self::STREAMER_VAR      => "availability",
                                                                       self::STREAMER_TYPE     => "SyncResolveRecipientsAvailability"];
        }

        if (Request::GetProtocolVersion() >= 14.1) {
            $mapping[SYNC_RESOLVERECIPIENTS_PICTURE]        = [  self::STREAMER_VAR      => "picture",
