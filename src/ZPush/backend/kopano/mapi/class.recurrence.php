...
class Recurrence extends BaseRecurrence
{
     ...
     */
    function createException($exception_props, $base_date, $delete = false, $exception_recips = [], $copy_attach_from = false)
    {
     ...
     */
    function modifyException($exception_props, $base_date, $exception_recips = [], $copy_attach_from = false)
    {
        if (isset($exception_props[$this->proptags["startdate"]]) && !$this->isValidExceptionDate($base_date, $this->fromGMT($this->tz, $exception_props[$this->proptags["startdate"]]))) {
            return false;
        }
    ...
    */
            ...
            if ($occTimeRange) {
                if ($occSingleDayRank) {
                    $pattern = sprintf(ngettext(
                        'Occurs every %s effective %s for %s occurence from %s to %s.',
                        'Occurs every %s effective %s for %s occurences from %s to %s.',
                        $numocc
                    ), $type, $start, $numocc, $startocc, $endocc);
                } else {
                    $pattern = sprintf(ngettext(
                        'Occurs every %s %s effective %s for %s occurence from %s to %s.',
                        'Occurs every %s %s effective %s for %s occurences %s to %s.',
                        $numocc
                ...
                if ($occSingleDayRank) {
                    $pattern = sprintf(ngettext(
                        'Occurs every %s effective %s for %s occurence.',
                        'Occurs every %s effective %s for %s occurences.',
                        $numocc
                    ), $type, $start, $numocc);
                } else {
                    $pattern = sprintf(ngettext(
                        'Occurs every %s %s effective %s for %s occurence.',
                        'Occurs every %s %s effective %s for %s occurences.',
                        $numocc
     ...
     */
    function createExceptionAttachment($exception_props, $exception_recips = [], $copy_attach_from = false)
    {
    ...
    function setDeltaExceptionRecipients($exception, $exception_recips, $copy_orig_recips)
    {
        ...
        $msgprops = mapi_getprops($exception, [PR_SENT_REPRESENTING_ENTRYID, PR_SENT_REPRESENTING_EMAIL_ADDRESS, PR_SENT_REPRESENTING_NAME, PR_SENT_REPRESENTING_ADDRTYPE, $this->proptags['responsestatus']]);
    ...
    function setAllExceptionRecipients($message, $exception_recips)
    {
        ...
        // Add organizer to meeting only if it is not organized.
        $msgprops = mapi_getprops($message, [PR_SENT_REPRESENTING_ENTRYID, PR_SENT_REPRESENTING_EMAIL_ADDRESS, PR_SENT_REPRESENTING_NAME, PR_SENT_REPRESENTING_ADDRTYPE, $this->proptags['responsestatus']]);
        if (isset($msgprops[$this->proptags['responsestatus']]) && $msgprops[$this->proptags['responsestatus']] != olResponseOrganized) {
