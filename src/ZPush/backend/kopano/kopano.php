...
class BackendKopano implements IBackend, ISearchProvider
{
    ...
    public function Logoff()
    {
        ...
        foreach ($this->importedFolders as $folderid => $store) {
            ...
            $storeprops = mapi_getprops($store, [PR_USER_ENTRYID]);
    ...
    public function SendMail($sm)
    {
        ...
        ZLog::Write(LOGLEVEL_DEBUG, sprintf(
            "KopanoBackend->SendMail(): RFC822: %d bytes  forward-id: '%s' reply-id: '%s' parent-id: '%s' SaveInSent: '%s' ReplaceMIME: '%s'",
            strlen($sm->mime),
        ...
        if (!$sendingAsSomeone) {
            mapi_deleteprops(
                $mapimessage,
                [  $sendMailProps["sentrepresentingname"], $sendMailProps["sentrepresentingemail"], $sendMailProps["representingentryid"],
    ...
    public function GetKoeGabBackendFolderId($foldername)
    {
        ...
        $restriction = [RES_PROPERTY, [RELOP => RELOP_EQ, ULPROPTAG => PR_DISPLAY_NAME, VALUE => $foldername]];
        mapi_table_restrict($table, $restriction);
        $querycnt = mapi_table_getrowcount($table);
        if ($querycnt == 1) {
            $entry = mapi_table_queryallrows($table, [PR_SOURCE_KEY]);
    ...
    public function GetGALSearchResults($searchquery, $searchrange)
    {
        ...
        if ($querycnt > 0) {
            $abentries = mapi_table_queryrows($table, [PR_ACCOUNT, PR_DISPLAY_NAME, PR_SMTP_ADDRESS, PR_BUSINESS_TELEPHONE_NUMBER, PR_GIVEN_NAME, PR_SURNAME, PR_MOBILE_TELEPHONE_NUMBER, PR_HOME_TELEPHONE_NUMBER, PR_TITLE, PR_COMPANY_NAME, PR_OFFICE_LOCATION], $rangestart, $querylimit);
        }
    ...
    public function GetMailboxSearchResults($cpo)
    {
        ...
        $rows = (is_array($searchRange) && isset($searchRange[0]) && isset($searchRange[1])) ?
            mapi_table_queryrows($table, [PR_SOURCE_KEY, PR_PARENT_SOURCE_KEY], $searchRange[0], $searchRange[1] - $searchRange[0] + 1) :
            mapi_table_queryrows($table, [PR_SOURCE_KEY, PR_PARENT_SOURCE_KEY], 0, SEARCH_MAXRESULTS);
    ...
    */
        ...
        mapi_table_restrict(
            ...
            [RES_CONTENT,
                [
                    ...
                    VALUE           => [PR_DISPLAY_NAME => "Z-Push Search Folder ".$pid]
                ]
            ],
            TBL_BATCH
        );
    ...
    public function GetKoeSignatures()
    {
        ...
        $storeProps = mapi_getprops($this->store, [PR_EC_WEBACCESS_SETTINGS_JSON]);
            ...
            if ($oof->oofstate == SYNC_SETTINGSOOF_GLOBAL && isset($oofprops[PR_EC_OUTOFOFFICE_FROM], $oofprops[PR_EC_OUTOFOFFICE_UNTIL])) {
                ...
                if ($now > $oofprops[PR_EC_OUTOFOFFICE_FROM] && $now > $oofprops[PR_EC_OUTOFOFFICE_UNTIL]) {
                    ...
                    ZLog::Write(LOGLEVEL_WARN, sprintf(
                        "BackendKopano->settingsOofGet(): Time based out of office set but end time ('%s') is before startime ('%s').",
                ...
                ZLog::Write(LOGLEVEL_WARN, sprintf(
                    "Kopano->settingsOofGet(): Time based out of office set but either start time ('%s') or end time ('%s') is missing.",
    ...
    */
    ...
    private function resolveRecipientGAL($to, $maxAmbiguousRecipients)
    {
        ...
        if ($querycnt > 0) {
            ...
            $abentries = mapi_table_queryrows($table, [PR_ENTRYID, PR_DISPLAY_NAME, PR_EMS_AB_TAGGED_X509_CERT, PR_OBJECT_TYPE], 0, $rowsToQuery);
    ...
    private function getAvailability($to, $resolveRecipient, $resolveRecipientsOptions)
    {
        ...
        $fbDataArray = mapi_freebusysupport_loaddata($fbsupport, [$resolveRecipient->id]);
    ...
    private function isZPushEnabled()
    {
        ...
        $userEntryid = mapi_getprops($this->store, [PR_MAILBOX_OWNER_ENTRYID]);
