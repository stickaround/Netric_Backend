  ...
  */
    function Render()
    {
      ...
      // and we can just return that without modification.
        ...
        } elseif ((strlen($property) + strlen($escaped)) > 72 && (strlen($property) < 72) && (strlen($escaped) < 72)) {
...
class iCalComponent
{
            ...
            if ($this->components[$j]->GetType() == $component_type) {
                for ($i = 0; $i < count($this->components[$j]->properties); $i++) {
                    if ($this->components[$j]->properties[$i]->Name() == $property_name) {
                        if (isset($this->components[$j]->rendered)) {
                            unset($this->components[$j]->rendered);
                        }
  ...
  */
    ...
    {
        if (isset($this->rendered) && count($new_properties) > 0) {
            unset($this->rendered);
        }
  ...
  */
    function IsAttendee($email)
    {
        ...
        if ($this->IsOrganizer($email)) {
            return true; /** an organizer is an attendee, as far as we're concerned */
  ...
  */
    ...
    {
        if (is_array($new_component) && count($new_component) == 0) {
            return;
        }
        if (isset($this->rendered)) {
            unset($this->rendered);
  ...
  */
    function CloneConfidential()
    {
        ...
        $keep_properties = [ 'DTSTAMP', 'DTSTART', 'RRULE', 'DURATION', 'DTEND', 'DUE', 'UID', 'CLASS', 'TRANSP', 'CREATED', 'LAST-MODIFIED' ];
        $resource_components = [ 'VEVENT', 'VTODO', 'VJOURNAL' ];
        $confidential->MaskComponents([ 'VTIMEZONE', 'VEVENT', 'VTODO', 'VJOURNAL' ]);
    ...
    function Render($restricted_properties = null)
    {
        ...
        foreach ($this->properties as $k => $v) {
            if (method_exists($v, 'Render')) {
                if ($unrestricted || isset($restricted_properties[$v])) {
                    $rendered .= $v->Render() . "\r\n";
                }
            }
        }
...
class iCalendar
{
  ...
  */
    function __construct($args)
    {
        ...
        if (isset($args['icalendar'])) {
            ...
            $this->properties['VCALENDAR'] = ['***ERROR*** This class is being referenced in an unsupported way!'];
            return;
        }
        ...
        $this->component->SetComponents([$first]);
  ...
  */
    function SaveTimeZones()
    {
        ...
        if ($timezones === false || count($timezones) == 0) {
            return;
        }
        ...
        if (isset($c->save_time_zone_defs) && $c->save_time_zone_defs) {
            foreach ($timezones as $k => $tz) {
                ...
                if ($tzid != "" && $qry->rows() == 0) {
                    ...
                    if (!isset($tzname)) {
                        $tzname = olson_from_tzstring($tzid);
                    }

                    $qry2 = new AwlQuery(
                        "INSERT INTO time_zone (tz_id, tz_locn, tz_spec) VALUES( ?, ?, ? );",
                        $tzid,
                        $tzname,
                        $tz->Render()
                    );
        ...
        if (! isset($this->tzid) && isset($first_tzid)) {
            $this->tzid = $first_tzid;
        }
        ...
        if (! isset($this->tzid) && isset($this->tz_locn)) {
            $this->tzid = $this->tz_locn;
        }
    }
  ...
  */
    function DefaultPropertyList()
    {
        ...
        return [ "UID" => 1, "DTSTAMP" => 1, "DTSTART" => 1, "DURATION" => 1,
        ...
        while (isset($this->lines[$this->_current_parse_line])) {
            ...
            if (preg_match('/^(BEGIN|END):([^:]+)$/', $line, $matches)) {
                if ($matches[1] == 'END' && $matches[2] == $type) {
                    ...
                    return $properties;
                } elseif ($matches[1] == 'BEGIN') {
                    $subtype = $matches[2];
                    if ($subtype == 'VTIMEZONE') {
                        $this->parsing_vtimezone = true;
                        $this->vtimezone = $line."\n";
                    }
                    if (!isset($properties['INSIDE'])) {
                        $properties['INSIDE'] = [];
                    }
  ...
  */
    function Set($key, $value)
    {
        ...
        if (isset($this->component->rendered)) {
            unset($this->component->rendered);
        }
        $component =& $this->component->FirstNonTimezone();
        $component->SetProperties([$property], $key);
        return $this->Get($key);
    }
  ...
  * @DEPRECATED: This class will be removed soon.
  ...
  //    dbg_error_log('iCalendar', ':TestFilter we have %d filters to test', count($filters) );
    ...
    //      dbg_error_log('iCalendar', ':TestFilter working on tag "%s" %s"', $k, $tag );
            ...
            if (! $this->ApplyFilter($filter, $value)) {
                return false;
            }
        }
        return true;
