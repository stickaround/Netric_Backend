...
class Kopano extends SyncWorker
{
    ...
    protected function createHiddenFolder($gabId = null, $gabName = 'default')
    {
        ...
        mapi_setprops($newfolder, [PR_CONTAINER_CLASS => "IPF.Appointment", PR_ATTR_HIDDEN => true]);
    ...
    */
    protected function getHiddenFolderId($gabId = null, $gabName = 'default')
    {
        ...
        $restriction = [RES_PROPERTY, [RELOP => RELOP_EQ, ULPROPTAG => PR_DISPLAY_NAME, VALUE => HIDDEN_FOLDERNAME]];
    ...
    protected function clearAllNotCurrentChunkType($folderid, $gabId = null, $gabName = 'default')
    {
        ...
        $restriction = [RES_PROPERTY, [RELOP => RELOP_NE, ULPROPTAG => $this->mapiprops['chunktype'], VALUE => $this->chunkType]];
        mapi_table_restrict($table, $restriction);
        $querycnt = mapi_table_getrowcount($table);
        if ($querycnt == 0) {
            ...
            $entries = mapi_table_queryallrows($table, [PR_ENTRYID, $this->mapiprops['chunktype']]);
    ...
    protected function getGAB($uniqueId = false, $gabId = null, $gabName = 'default')
    {
        ...
        if ($uniqueId) {
            ...
            $restriction = [RES_PROPERTY, [RELOP => RELOP_EQ, ULPROPTAG => $prop , VALUE => $uniqueId]];
            mapi_table_restrict($table, $restriction);
            $querycnt = mapi_table_getrowcount($table);
            if ($querycnt == 0) {
        ...
        foreach ($gabentries as $entry) {
            ...
            if (array_key_exists($entry[PR_ACCOUNT], $groups)) {
                ...
                $users = mapi_table_queryallrows($grouptable, [PR_ENTRYID, PR_ACCOUNT, PR_SMTP_ADDRESS]);
    ...
    private function findChunk($store, $folderid, $chunkName)
    {
        ...
        $restriction = [RES_PROPERTY, [RELOP => RELOP_EQ, ULPROPTAG => PR_SUBJECT, VALUE => $chunkName]];
        mapi_table_restrict($table, $restriction);

        $entries = mapi_table_queryallrows($table, [PR_ENTRYID, $this->mapiprops['chunkCRC']]);
        if (isset($entries[0])) {
            return $entries[0];
        }
        return [];
    ...
    private function getRootFolder($store)
    {
        ...
        if (!isset($this->folderCache[$rootId])) {
            ...
            if ($this->mainUser == HIDDEN_FOLDERSTORE && strtoupper($this->mainUser) != "SYSTEM") {
                $parentprops = mapi_getprops($store, [PR_IPM_SUBTREE_ENTRYID]);
            ...
            // get the main public folder
                ...
                $parentprops = mapi_getprops($store, [PR_IPM_PUBLIC_FOLDERS_ENTRYID]);
